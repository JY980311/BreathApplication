package com.example.breathapplication.asleep.viewmodel
...
class AsleepViewModel @Inject constructor(
    ...
    fun initAsleepConfig(userId: String?) {
        Asleep.initAsleepConfig(
            ...
            apiKey = BuildConfig.ASLLEP_API_KEY,
            userId = userId,
            baseUrl = null,
            callbackUrl = null,
            service = "SampleApp",
    ...
    fun continueTracking() {
        if (_asleepConfig == null && Asleep.hasUnfinishedSession(applicationContext)) { // Conditions for continuing
            _asleepConfig = Asleep.getSavedAsleepConfig(applicationContext, BuildConfig.ASLLEP_API_KEY)
